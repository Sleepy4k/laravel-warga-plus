<x-layouts.dashboard title="Product Category">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <p class="mb-4">
            <strong>Categories</strong> help you organize your products for easier management and reporting.<br />
        ...
        <x-dashboard.canvas.wrapper canvasId="add-new-record" canvasPermission="product.category.create">
            ...
            <x-dashboard.canvas.body>
                <form class="add-new-record pt-0 row g-2" id="form-add-new-record" method="POST"
                    action="{{ route('dashboard.product.category.store') }}">
                    <div class="col-sm-12">
                        <label class="form-label" for="name">Name</label>
                        <div class="input-group input-group-merge">
                            <input type="text" id="name"
                                class="form-control dt-name @error('name') is-invalid @enderror" name="name"
                                placeholder="kuliner_lokal" aria-label="kuliner_lokal" aria-describedby="name"
                                value="{{ old('name') }}" />
                        </div>
                    </div>
                    <div class="col-sm-12">
                        ...
                        <div class="input-group input-group-merge">
                            <input type="text" id="label" name="label"
                                ...
                                placeholder="Kuliner Lokal" aria-label="Kuliner Lokal" aria-describedby="label"
                                value="{{ old('label') }}" />
                        </div>
                    </div>
                </form>
        ...
        <x-dashboard.canvas.wrapper canvasId="show-record" canvasPermission="product.category.show">
                    ...
                    <div class="col-sm-12">
                        ...
                        <div class="input-group input-group-merge">
                            <input type="text" id="show-name" class="form-control dt-name"
                                placeholder="kuliner_lokal" aria-label="kuliner_lokal" aria-describedby="show-name"
                                disabled />
                        </div>
                    </div>
                    <div class="col-sm-12">
                        ...
                        <div class="input-group input-group-merge">
                            <input type="text" id="show-label" class="form-control dt-description"
                                placeholder="Kuliner Lokal" aria-label="Kuliner Lokal" aria-describedby="show-label"
                                disabled />
                        </div>
                    </div>
                </div>
    ...
    @pushOnce('page-scripts')
        ...
        @canany(['article.category.create', 'article.category.edit'])
            <script type="text/javascript" src="{{ asset('js/pages/product-category.min.js') }}" @cspNonce></script>
        @endcanany
        @canany(['product.category.create', 'product.category.edit', 'product.category.show', 'product.category.delete'])
            ...
            <script @cspNonce>
                $(document).ready(function() {
                    const crudManager = new CrudManager({
                        ...
                        routes: {
                            update: "{{ route('dashboard.product.category.update', ':id') }}",
                            destroy: "{{ route('dashboard.product.category.destroy', ':id') }}"
                        },
                        offcanvas: {
                            show: {
                                id: '#show-record',
