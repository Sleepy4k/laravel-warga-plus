<x-layouts.dashboard title="Activity Category">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <p class="mb-4">
            <strong>Categories</strong> help you organize your activities for easier management and reporting.<br />
        ...
        <x-dashboard.canvas.wrapper canvasId="add-new-record" canvasPermission="activity.category.create">
            ...
            <x-dashboard.canvas.body>
                <form class="add-new-record pt-0 row g-2" id="form-add-new-record" method="POST"
                    action="{{ route('dashboard.activity.category.store') }}">
                    <div class="col-sm-12">
        ...
        <x-dashboard.canvas.wrapper canvasId="show-record" canvasPermission="activity.category.show">
        ...
        <x-dashboard.canvas.wrapper canvasId="edit-record" canvasPermission="activity.category.edit">
    ...
    @pushOnce('page-scripts')
        ...
        @canany(['activity.category.create', 'activity.category.edit'])
            <script type="text/javascript" src="{{ asset('js/pages/activity-category.min.js') }}" @cspNonce></script>
        @endcanany
        @canany(['activity.category.create', 'activity.category.edit', 'activity.category.show', 'activity.category.delete'])
            <script type="text/javascript" src="{{ asset('js/handler/crud-manager.min.js') }}" @cspNonce></script>
            <script @cspNonce>
                $(document).ready(function() {
                    const crudManager = new CrudManager({
                        debug: "{{ config('app.debug') }}",
                        tableId: '#activity-category-table',
                        csrfToken: '{{ csrf_token() }}',
                        routes: {
                            update: "{{ route('dashboard.activity.category.update', ':id') }}",
                            destroy: "{{ route('dashboard.activity.category.destroy', ':id') }}"
                        },
                        offcanvas: {
                            show: {
                                id: '#show-record',
