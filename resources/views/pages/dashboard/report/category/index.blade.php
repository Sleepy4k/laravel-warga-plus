<x-layouts.dashboard title="Report Category">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <x-dashboard.canvas.wrapper canvasId="add-new-record" canvasPermission="report.category.create">
            ...
            <x-dashboard.canvas.body>
                <form class="add-new-record pt-0 row g-2" id="form-add-new-record" method="POST"
                    action="{{ route('dashboard.report.category.store') }}">
                    <div class="col-sm-12">
        ...
        <x-dashboard.canvas.wrapper canvasId="edit-record" canvasPermission="report.category.edit">
    ...
    @pushOnce('page-scripts')
        ...
        @canany(['report.category.create', 'report.category.edit'])
            <script type="text/javascript" src="{{ asset('js/pages/report-category.min.js') }}" @cspNonce></script>
        @endcanany
        @canany(['report.category.create', 'report.category.edit', 'report.category.show', 'report.category.delete'])
            <script type="text/javascript" src="{{ asset('js/handler/crud-manager.min.js') }}" @cspNonce></script>
            <script @cspNonce>
                $(document).ready(function() {
                    const crudManager = new CrudManager({
                        debug: "{{ config('app.debug') }}",
                        tableId: '#report-category-table',
                        csrfToken: '{{ csrf_token() }}',
                        routes: {
                            update: "{{ route('dashboard.report.category.update', ':id') }}",
                            destroy: "{{ route('dashboard.report.category.destroy', ':id') }}"
                        },
                        offcanvas: {
                            show: {
                                id: '#show-record',
