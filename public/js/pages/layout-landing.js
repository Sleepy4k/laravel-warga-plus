(() => {
  ...
  if ("1200" <= screen.width && t) {
    t.addEventListener("mousemove", function(event) {
      ...
      r.forEach(element => {
        ...
        element.style.transform = `perspective(1200px) rotateX(${(window.innerHeight - 2 * event.pageY) / 100}deg) rotateY(${rotateY}deg) scale3d(1, 1, 1)`
      });
    });
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    const monthlyPrices = [].slice.call(document.querySelectorAll(".price-monthly"));
    const yearlyPrices = [].slice.call(document.querySelectorAll(".price-yearly"));
