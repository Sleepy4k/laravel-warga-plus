...
class SystemDataTable extends DataTable
{
    ...
    {
        return collect(
            (object) LogReader::getFileList(LogReaderType::DAILY)
        );
    }
    ...
    public function dataTable(): CollectionDataTable
    {
        return datatables()
            ...
            ->addColumn('action', function ($query) {
                if (!canUserPerformAction('log.system.show')) {
                    return '-';
                }

                $name = explode('.', $query['name'])[0];
                $date = substr($name, 8, 10);

                return '<a href="' . route("dashboard.log.system.show", $date) . '"><button class="btn btn-info btn-sm me-1 mb-1"><i class="fa-solid fa-eye"></i></button></a>';
            })
            ->editColumn('name', function ($query) {
                $name = explode('.', $query['name'])[0];
                $date = substr($name, 8, 10);
                return 'Laravel - ' . date('d M Y', strtotime($date));
            })
