...
class QueryDataTable extends DataTable
{
    ...
    {
        return collect(
            (object) LogReader::getFileList(LogReaderType::DAILY, 'query')
        );
    }
    ...
    public function dataTable(): CollectionDataTable
    {
        return datatables()
            ...
            ->addColumn('action', function ($query) {
                if (!canUserPerformAction('log.query.show')) {
                    return '-';
                }
                ...
                return '<a href="' . route("dashboard.log.query.show", $date) . '"><button class="btn btn-info btn-sm me-1 mb-1"><i class="fa-solid fa-eye"></i></button></a>';
            })
            ->editColumn('name', function ($query) {
                $name = explode('.', $query['name'])[0];
                $date = substr($name, 6, 10);
                return 'Query - ' . date('d M Y', strtotime($date));
            })
